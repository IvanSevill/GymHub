...
const Login: React.FC = () => {
  ...
  const googleLogin = useGoogleLogin({
    onSuccess: async (codeResponse) => {
      ...
      setError(null);
      try {
        const { access_token, user } = await authService.loginWithGoogle(codeResponse.code);
        login(access_token, user);
        navigate('/');
      } catch (err: any) {
        console.error('Login failed:', err);
        setError(err.response?.data?.detail || 'Error al autenticar con Google. Revisa tu conexión.');
      } finally {
    ...
    <div className="flex items-center justify-center min-h-screen bg-background dot-pattern px-4 relative overflow-hidden">
      ...
      >
        <div className="flex flex-col items-center mb-12">
          ...
          <h1 className="text-5xl font-black text-white tracking-tighter mb-4">Gym<span className="text-primary">Hub</span></h1>
      ...
      <div className="absolute bottom-10 text-[10px] font-black text-slate-700 uppercase tracking-[0.5em] pointer-events-none">
        Protocolo v2.0 • 2026
      </div>
    </div>
  );
