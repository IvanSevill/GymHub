...
import { TrendingUp, BarChart2, Zap, ArrowUpRight, Scale, Clock, Award, ChevronRight, X } from 'lucide-react';
...
const Analytics: React.FC = () => {
  ...
  const [viewType, setViewType] = useState('muscle'); // muscle, exercise
  const [frequencyData, setFrequencyData] = useState<ExerciseFrequency[]>([]);
  ...
  useEffect(() => {
    const init = async () => {
      try {
        ...
        setExercises(exRes);
        setFrequencyData(freqRes);
        
        if (wRes.length > 0) {
          const last = wRes[0];
          setLastWorkout(last);
          
          const lastMuscles = new Set(last.exercise_sets.map(s => s.exercise?.muscle?.name));
          const previousMatch = wRes.slice(1).find(w => 
            w.exercise_sets.some(s => lastMuscles.has(s.exercise?.muscle?.name))
          );
          ...
          if (previousMatch) {
            ...
            setDelta({
              ...
              date: format(parseISO(previousMatch.start_time), 'dd MMM', { locale: es }),
              improved: volDelta >= 0
            });
          }
        }
        if (exRes.length > 0) setSelectedExercise(exRes[0].id);

        const maxLiftsRes = await analyticsService.getMaxLifts();
        setMaxLifts(maxLiftsRes);
      } catch (err) {
        console.error(err);
      } finally {
        setLoading(false);
  ...
  useEffect(() => {
    if (selectedExercise) {
      const fetchWeight = async () => {
        try {
          const res = await analyticsService.getWeightProgress(selectedExercise, period);
          setWeightData(res.map(d => ({
            ...d,
  ...
  const fetchHistory = async (exerciseId: string, exerciseName: string) => {
    try {
      const res = await analyticsService.getExerciseHistory(exerciseId);
      setHistoryData(res);
  ...
  const chartData = viewType === 'muscle' 
    ? Object.entries(frequencyData.reduce((acc: any, curr) => {
        const m = curr.muscle_name || 'Otro';
        acc[m] = (acc[m] || 0) + curr.count;
        return acc;
      }, {})).map(([name, count]: any) => ({ name, count })).sort((a: any, b: any) => b.count - a.count)
            ...
            <div className="bg-gradient-to-r from-primary/10 to-transparent p-6 md:p-8 flex flex-col md:flex-row justify-between items-start md:items-center gap-6">
              <div className="flex items-center gap-6">
                ...
                <div>
                   ...
                   <div className="flex items-center gap-3 text-slate-400 text-xs font-bold mt-1">
                     <span className="flex items-center gap-1.5"><Clock size={12}/> {format(parseISO(lastWorkout.start_time), 'dd MMM', { locale: es })}</span>
                     <span className="text-white/10">|</span>
                     <span>{lastWorkout.exercise_sets.length} Series Totales</span>
                   </div>
                   
                   <div className="mt-4 flex flex-wrap gap-2 max-w-md">
                      {Array.from(new Set(lastWorkout.exercise_sets.map(s => s.exercise?.name).filter(Boolean))).slice(0, 4).map(ex => (
              ...
              <div className="flex gap-4">
                {lastWorkout.fitbit_data && (
                  <>
                    <div className="text-center bg-black/20 px-4 py-2 rounded-2xl border border-white/5">
            ...
            <>
              <div className="mb-8">
                <select 
                  ...
                  className="w-full sm:w-64 bg-surface border border-white/10 rounded-2xl px-5 py-4 text-sm font-bold text-white outline-none focus:border-primary/50 cursor-pointer"
                >
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 max-h-[450px] overflow-y-auto pr-2 no-scrollbar">
              {maxLifts.map((lift) => (
                ...
                >
                  ...
                  <p className="text-[9px] font-bold text-slate-600 mt-2">Logrado el {format(parseISO(lift.date), 'dd/MM/yyyy')}</p>
                </div>
              ))}
            </div>
          )}
               ...
               <div className="max-h-[300px] overflow-y-auto no-scrollbar">
                  <table className="w-full text-left">
                    <thead className="sticky top-0 bg-[#0c1221] z-10">
                      <tr>
                        <th className="px-6 py-4 text-[9px] font-black text-slate-600 uppercase tracking-widest border-b border-white/5">Elemento</th>
                        <th className="px-6 py-4 text-[9px] font-black text-slate-600 uppercase tracking-widest text-right border-b border-white/5">Sets Totales</th>
                      </tr>
                    </thead>
                      ...
                      {(viewType === 'muscle' 
                        ...
                        <tr key={i} className="hover:bg-white/[0.02] transition-colors group">
                          <td className="px-6 py-4 flex items-center gap-3">
                            ...
                            <span className="text-sm font-bold text-slate-300 group-hover:text-white capitalize">{name}</span>
                          </td>
          ...
          <div className="fixed inset-0 z-[120] flex items-center justify-center p-4">
            ...
            >
              <div className="p-6 border-b border-white/5 flex justify-between items-center bg-white/[0.02]">
                <div>
                  <h3 className="text-xl font-black text-white capitalize">{selectedHistory.name}</h3>
                ...
                {historyData.map((h, i) => (
                  <div key={i} className="flex justify-between items-center bg-white/[0.02] border border-white/5 p-4 rounded-2xl group hover:border-primary/30 transition-all">
                    <div className="flex flex-col">
                      <span className="text-[10px] font-black text-slate-500 uppercase tracking-widest">{format(parseISO(h.date), 'dd/MM/yyyy')}</span>
                      <span className="text-xs font-bold text-slate-400">{format(parseISO(h.date), 'HH:mm')}</span>
                    </div>
                    <div className="flex items-baseline gap-1">
